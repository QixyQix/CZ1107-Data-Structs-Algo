...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    newNode->key = key;
    //Set next to the LL current head
    ListNode *oldHead = Q1Ptr->Table[index].head;
...
{
    //Write your code here
    ListNode *toDelete = HashSearch(*Q1Ptr, key);
    ...
    //Remove the node
    int index = Hash(key,Q1Ptr->hSize);
    ListNode *pre = toDelete->pre;
    ListNode *aft = toDelete->next;
    ...
    if(Q1Ptr->Table[index].head == toDelete){
        Q1Ptr->Table[index].head = aft;
    }

...
void removeDuplicatesSortedLL(LinkedList *ll)
{
	...
	while (currNode != NULL)
	{
		//Check if the currentNode and the nextNode are the same
		if (currNode->next != NULL)
		{
			if (currNode->next->item == currNode->item)
			{

				//Duplicate is at the start, adjust head
				if (prevNode == NULL)
				{

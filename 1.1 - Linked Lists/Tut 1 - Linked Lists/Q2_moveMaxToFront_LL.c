...
int moveMaxToFront(ListNode **ptrHead)
{
	...
	ListNode *nodeBeforeMax = NULL;

	int currentMax = 0;

	//Iterate through LL, store the prevNode of the max in nodeBeforeMax
	while(currNode->next != NULL){
		if(currNode->item > currentMax){
			currentMax = currNode->item;
			nodeBeforeMax = prevNode;
		}
	...
	//If this is NULL, the max is already at the front
	if(nodeBeforeMax != NULL){
		ListNode *maxNode = nodeBeforeMax->next;

...

{
    ...
    //Base Case #2: Check either tree are null --> no match
    if(tree1 == NULL && tree2 != NULL || tree1 != NULL && tree2 == NULL){
        return 0;
    }
    //Base Case #3: Contents do not match
    if (tree1->item != tree2->item)
    {
        return 0;
    }

    //Iterative step: Call same function to determine if child trees are identical
    int leftResult, rightResult;
    leftResult = identical(tree1->left,tree2->left);
    rightResult = identical(tree1->right,tree2->right);

    //Return no match if no match is found

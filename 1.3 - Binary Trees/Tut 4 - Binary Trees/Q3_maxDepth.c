...
int maxDepth(BTNode *node)
{
    /* add your code here */
    //Recursive function
    //Node == Null, reached end of a path
    //Return -1 to negate the +1 effect from returned value
    if(node == NULL){
        return -1;
    }

    //Compute the max depth of the left and right node
    int leftDepth = maxDepth(node->left);
    int rightDepth = maxDepth(node->right);

    //Add 1 to the larger depth

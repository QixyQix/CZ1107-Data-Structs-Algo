...
BTNode *searchNode(BTNode *node, int key)
{
    /* add your code here */
    //Recursive function
    //Base case 1: node empty
    if(node == NULL){
        return NULL;
    }
    //Base case 2: node matches, return node
    if(node->item == key){
        return node;
    }

    //Recursively call searchNode to find the node
    BTNode *leftResult = searchNode(node->left,key);
    BTNode *rightResult = searchNode(node->right, key);
    //If leftResult is null, return the right result
    if(leftResult == NULL)
        return rightResult;
    else
        return leftResult;

...
void levelOrderTraversal(BSTNode *root)
{
	...
	if (root != NULL)
	{
		...
		{
			//a) Dequeue node and print it
			temp = dequeue(&queue.head, &queue.tail);
			printf("%d ", temp->item);
			//b) Enqueue current node's left child
			if (temp->left != NULL)
				enqueue(&queue.head, &queue.tail, temp->left);
			//c) Enqueue current node's right child
			if (temp->right != NULL)
				enqueue(&queue.head, &queue.tail, temp->right);
		}
	}
}
